...
최종 제출을 위해 서비스의 완성도를 높이는 5가지 핵심 개선 작업을 수행했습니다.
...
### 5. 계정 관리 고도화 (Advanced Account Management)
- **Soft vs Hard Delete**: 회원 탈퇴 시 '모든 기록 삭제' 옵션을 선택할 수 있습니다.
    - **체크 해제 시 (Default)**: 로그아웃만 수행되며, 데이터는 보존되어 재로그인 시 이어서 사용할 수 있습니다.
    - **체크 시 (Hard Delete)**: 서재, 독서 기록, 대화 내용 등 모든 개인 데이터가 영구적으로 삭제되며, 재가입 시 온보딩부터 다시 시작합니다.
